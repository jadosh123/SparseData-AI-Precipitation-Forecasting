...
def sample_silver_data():
    ...
    """
    try:
        return pd.read_sql("SELECT * FROM clean_station_data", engine)
    except Exception as e:
        pytest.fail(f"Database connection failed. Ensure Docker is running.\nError: {e}")

def test_no_negative_rain(sample_silver_data):
    """
    Integrity Check: Rain in the silver layer must not be negative after cleaning.
    """
    valid_rain = sample_silver_data["rain"].dropna()
...
def test_temperature_absolute_bounds(sample_silver_data):
    """
    ...
    for col in cols_to_check:
        valid_temps = sample_silver_data[col].dropna()
...
def test_temp_consistency_logic(sample_silver_data):
    """
    ...
    """
    max_violation = sample_silver_data[sample_silver_data['td'] > sample_silver_data['tdmax']]
    assert max_violation.empty, \
        f"Logic Error: Found {len(max_violation)} rows where T_current > T_max.\n{max_violation}"

    min_violation = sample_silver_data[sample_silver_data['td'] < sample_silver_data['tdmin']]
    assert min_violation.empty, \
        f"Logic Error: Found {len(min_violation)} rows where T_current < T_min.\n{min_violation}"

    inverted_range = sample_silver_data[sample_silver_data['tdmax'] < sample_silver_data['tdmin']]
